...
// Alternative version with gradient number effect (more Netflix-like)
class MovieImageButtonGradient extends StatelessWidget {
  final String imagePath;
  final int number;
  final VoidCallback onTap;

  const MovieImageButtonGradient({
    super.key,
    required this.imagePath,
    required this.number,
    required this.onTap,
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        height: 210,
        width: 150, // Slightly wider to accommodate the number
        child: Stack(
          children: [
            // Movie poster image
            Positioned(
              left: 25, // Offset to make room for the number
              top: 0,
              bottom: 0,
              right: 0,
              child: Container(
            ...
            // Ranking number with gradient and stroke
            Positioned(
              left: 0,
              bottom: 8,
              child: Stack(
